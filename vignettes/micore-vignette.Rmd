...
It's also possible to directly extracted the estimated covariance matrix based on a covariate profile $\xb_i$.  The function \texttt{getPredCov()} allows the user to provide a covariate profile and will return the corresponding estimated covariance, precision, correlation, or partial correlation matrix along with interval estimates.
```{r abund}
